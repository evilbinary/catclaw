...
bool weixin_send_message(const char *bot_token, const char *to_user_id,
    ...
    if (!bot_token || !to_user_id || !text) {
        log_error("[Weixin] send_message: invalid params");
        return false;
    }
    
    log_info("[Weixin] Sending message to %s: %s", to_user_id, text);
    ...
    if (!body) {
        log_error("[Weixin] Failed to create JSON body");
        return false;
    }
    
    log_debug("[Weixin] Send message body: %s", body);
    ...
    if (!resp->success) {
        log_error("[Weixin] Failed to send message, status: %d", resp->status_code);
        http_response_free(resp);
        return false;
    }
    ...
    if (!json) {
        log_error("[Weixin] Failed to parse send response");
        return false;
    }
    ...
    if (ret) {
        if (cJSON_IsNumber(ret)) {
            success = (ret->valueint == 0);
        }
        if (!success) {
            log_error("[Weixin] Send message returned ret=%d", ret->valueint);
        }
    }
    
    if (success) {
        log_info("[Weixin] Message sent successfully");
    }
